...
CATEGORY_OVERPASS: dict[str, str] = {
    ...
    "Hotels & Stay":   'node["amenity"~"hotel|guest_house|hostel|lodge"]\n  node["tourism"~"hotel|guest_house"]',
    "Sightseeing":     'node["tourism"~"attraction|museum|viewpoint|artwork"]',
    "Parks & Leisure": 'node["leisure"~"park|garden|stadium"]',
    "Healthcare":      'node["amenity"~"hospital|clinic|pharmacy|doctors"]',
    "Worship":         'node["amenity"="place_of_worship"]',
    "Shopping":        'node["shop"~"mall|supermarket|department_store"]',
}

# Lighter query — nodes only, no way/relation, much faster
ALL_CATEGORIES_QUERY = """
  node["amenity"~"restaurant|cafe|fast_food|food_court|bar"]{around};
  node["amenity"~"hotel|guest_house|hostel|lodge"]{around};
  node["amenity"~"hospital|clinic|pharmacy|doctors"]{around};
  node["amenity"="place_of_worship"]{around};
  node["tourism"~"attraction|museum|viewpoint|artwork|hotel|guest_house"]{around};
  node["leisure"~"park|garden|stadium"]{around};
  node["shop"~"mall|supermarket|department_store"]{around};
"""
...
def _build_overpass_query(lat: float, lon: float, radius: int, category) -> str:
...
def fetch_overpass(query: str) -> list:
    ...
    while pending:
        done, pending = wait(pending, timeout=50, return_when=FIRST_COMPLETED)
        for future in done:
